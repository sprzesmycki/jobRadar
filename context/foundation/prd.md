...
- Użytkownik wgrywa CV, ustawia preferencje i widzi zagregowaną listę ofert z trzech portali (JustJoinIT, Remotive, Adzuna) przefiltrowaną po roli, technologiach i widełkach.
...
- **Then** widzi zagregowaną listę ofert ze skonfigurowanych portali (JustJoinIT, Remotive, Adzuna) przefiltrowaną po roli, technologiach i min. widełkach — każda z wynikiem % dopasowania CV oraz listą brakujących skills
...
- FR-004: User can set job preferences: role, technologies, min salary, work mode (remote/hybrid/onsite). Priority: must-have
  > Socrates: Kontrargument rozważony: "match score już filtruje — preferences redundantne." Pominięcie: preferencje są niezbędne do pre-filtrowania przed scoringiem — bez nich fetches i scoring uruchamiają się dla tysięcy irrelevantnych ofert.
...
- Czas generowania cover lettera nie przekracza 30 sekund od momentu kliknięcia przez użytkownika; dla każdej operacji trwającej dłużej niż 2 sekundy użytkownik otrzymuje ciągły, widoczny feedback o postępie operacji.
- Pełna treść CV użytkownika nie jest przechowywana przez serwisy zewnętrzne po zakończeniu zapytania — żaden zewnętrzny log nie może zawierać surowego tekstu CV.
...
Użytkownik loguje się przez email + hasło lub OAuth za pośrednictwem zewnętrznego dostawcy tożsamości. Dane profilu (CV, preferencje) przechowywane w koncie użytkownika po stronie aplikacji. MVP: zamknięta rejestracja — jeden użytkownik (właściciel aplikacji) weryfikuje produkt zanim otworzy innym. Brak ról w MVP — jeden poziom dostępu.
...
- Nie buduje własnego modelu matchingu: scoring i generowanie cover lettera opierają się na gotowych zewnętrznych usługach; żadnego własnego trenowania modeli, fine-tuningu ani ML pipeline.
...
Brak otwartych pytań po zakończeniu procesu shapingu. Wszystkie kluczowe decyzje dotyczące zakresu produktu zostały podjęte w trakcie sesji /10x-shape.
